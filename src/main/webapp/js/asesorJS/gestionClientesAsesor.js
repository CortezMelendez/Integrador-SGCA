// ==========================================================
// Gestión de Clientes (Asesor) — mismo patrón que duenioJS/gestionClientes.js,
// pero sin la columna/modal "Asesor": aquí la cartera siempre es la del
// asesor logueado, resuelta en el servidor.
// ==========================================================
...
function confirmarEliminarUsuario(id) {
  pedirConfirmacion('¿Deseas eliminar este cliente? Esta acción no se puede deshacer.', () => {
    window.location.href = `${CONTEXT_PATH}/gestionClienteAsesor?accion=eliminar&id=${id}`;
  }, 'peligro');
}

// Agregar un cliente disponible (sin asesor) a la propia cartera
function confirmarAsignarAsesor(idCliente) {
  pedirConfirmacion('¿Deseas agregar este cliente a tu cartera?', () => {
    window.location.href = `${CONTEXT_PATH}/gestionClienteAsesor?accion=asignarAsesor&idCliente=${idCliente}`;
  });
}
...
function toggleEstado(elemento, idUsuario) {
  ...
  fetch(`${CONTEXT_PATH}/gestionClienteAsesor?accion=cambiarEstado&id=${idUsuario}&estado=${nuevoEstado}`)
