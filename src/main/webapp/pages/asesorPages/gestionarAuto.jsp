...
<%-- Errores de la acción "eliminar" (redirect por GET, no pasa por
     mostrarListado con un mensaje de formulario, así que se lee directo
     de la query string en vez de errorParam). Eliminar autos es exclusivo
     del dueño; esto solo se dispararía si alguien intenta la URL directo. --%>
<c:if test="${not empty param.error}">
    <script>
        window.addEventListener('DOMContentLoaded', () => {
            const mensajesEliminar = {
                no_autorizado: 'Solo el dueño puede eliminar vehículos. Puedes desactivarlo desde el estado.'
            };
            const codigoEliminar = '${param.error}';
            if (mensajesEliminar[codigoEliminar]) {
                alert(mensajesEliminar[codigoEliminar]);
            }
        });
    </script>
</c:if>
