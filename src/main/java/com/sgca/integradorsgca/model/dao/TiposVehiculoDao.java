...
public class TiposVehiculoDao {
    ...
    // Busca un tipo de vehículo por nombre (ej. "SUV", "Sedán")
    public TiposVehiculoBean buscarPorNombre(String nombre) {
        String sql = "SELECT ID_TIPO, NOMBRE FROM ADMIN.TIPOS_VEHICULO WHERE UPPER(NOMBRE) = UPPER(?)";
        TiposVehiculoBean tipo = null;
        ...
        try (Connection con = Conexion.getConexion();
            ...
            System.err.println("Error al buscar tipo de vehículo por nombre: " + e.getMessage());
        }
        return tipo;
    }

    // Devuelve el ID del tipo; si no existe, lo crea
    public int obtenerOCrearId(String nombre) {
        TiposVehiculoBean existente = buscarPorNombre(nombre);
        if (existente != null) return existente.getIdTipo();

        registrar(new TiposVehiculoBean(nombre));

        TiposVehiculoBean creado = buscarPorNombre(nombre);
        return creado != null ? creado.getIdTipo() : -1;
    }
